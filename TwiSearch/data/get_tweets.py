...
def get_tweets(word, total):
 
    getter = TweetsGetter.bySearch(word)
    
    with open('data/'+word+'.txt', 'w', encoding='utf-8') as f1, open('data/'+word+'_リンク.txt', 'w', encoding='utf-8') as f2, open('data/'+word+'_detail.txt', 'w', encoding='utf-8') as f3:        
      for tweet in getter.collect(total = total):
        f1.write(str(tweet['text']))
        f1.write('＝＝＝＝＝＝＝＝＝＝')

        f2.write('https://twitter.com/'+str(tweet['user']['screen_name'])+'/status/'+str(tweet['id']))
        f2.write('＝＝＝＝＝＝＝＝＝＝')
        f2.write('\n')

        f3.write(str(tweet['created_at'])+' ')
        f3.write(str(tweet['id'])+ '\n')
        f3.write(tweet['text'])
        f3.write('\n')
        f3.write('＝＝＝＝＝＝＝＝＝＝')
        f3.write('\n')